...
S.tool("why_did_it_move",
       ...
       obj({"instrument": _str("the instrument that moved"),
...
S.tool("fit_factor_model",
       ...
       obj({"returns_csv": _str("one 'instrument,market,sector' row per line")},
           ["returns_csv"]),
       )(T.fit_factor_model)

S.tool("compose_thesis",
       ...
       obj({"instrument": _str("the subject"),
            "evidence": {"type": "array", "description":
                         "[{'agent':'a1_fundamentals','text':'...'}]",
                         "items": obj({"agent": _str("which evidence agent"),
                                       "text": _str("the finding")},
                                      ["agent", "text"])},
            "breakers": {"type": "array", "description":
                         "what would prove this WRONG, each machine-checkable",
                         "items": obj({"statement": _str("the falsifier in words"),
...
S.tool("check_portfolio_risk",
       ...
       obj({"positions": {"type": "array", "items": obj(
...
S.tool("size_position",
       ...
       obj({"instrument": _str("e.g. 'MYX:1155'"),
...
S.tool("plan_question",
       ...
       obj({"question": _str("the question in plain words"),
            "instruments": {"type": "array", "items": {"type": "string"}},
            "budget_myr": _num("cap; too small is refused, not cheapened")},
           ["question"]),
       )(T.plan_question)

S.tool("explain_concept",
...
def selftest() -> int:
    ...
    print(f"protocol      {lines[0]['result']['protocolVersion']}")
    print(f"server        {lines[0]['result']['serverInfo']}")
    print(f"tools         {len(lines[1]['result']['tools'])}")
    for t in lines[1]["result"]["tools"]:
        print(f"                {t['name']}")
    print(f"responses     {len(lines)} for {len([r for r in reqs if 'id' in r])} requests "
          f"(the notification correctly got none)")
    ok = (len(lines) == 3 and "error" not in lines[2]
...
def main(argv=None) -> int:
    ...
    if "--list" in argv:
        for t in S.tools.values():
            print(f"{t.name:<24} {t.description.splitlines()[0]}")
        return 0
    return S.serve()
