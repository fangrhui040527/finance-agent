...
def test_a_deterministic_rebuild_leaves_another_tier_alone(tmp_path):
    with GraphStore(tmp_path / "g.db") as s:
        ...
        assert all(e.confidence is Confidence.EXTRACTED for e in s.load(tier=DETERMINISTIC).edges())
...
def hub_graph(n_leaves, hub_kind=NodeKind.COUNTRY):
    ...
    for i in range(n_leaves):
        ...
        g.add_edge(
            Edge(f"CO:{i}", "HUB", EdgeKind.OPERATES_IN, 1.0, "d", Confidence.EXTRACTED, OPENED)
        )
        g.add_edge(Edge("HUB", f"CO:{i}", EdgeKind.AFFECTS, 1.0, "d", Confidence.EXTRACTED, OPENED))
    return g
...
def test_the_hub_set_is_recomputed_when_an_edge_arrives():
    ...
    g.add_edge(
        Edge("CO:extra", "HUB", EdgeKind.OPERATES_IN, 1.0, "d", Confidence.EXTRACTED, OPENED)
    )
    g.add_edge(Edge("HUB", "CO:extra", EdgeKind.AFFECTS, 1.0, "d", Confidence.EXTRACTED, OPENED))
...
def test_rebuild_starts_from_an_empty_file(tmp_path):
    ...
    with GraphStore(p) as s:
        s.close_edge(
            "CO:XKLS:1155", "CO:XKLS:1023", EdgeKind.COMPETES_WITH, OPENED, date(2026, 1, 1)
        )
...
def a7_over(graph, corpus):
    ...
    ctx = AgentContext(
        router=Router({}), engine=default_engine({"a7_sector_technology": {"traverse"}})
    )
    ...
    missing = sorted(
        {
            ...
            if e.source_doc_id.startswith("curated:") and corpus.citation(e.source_doc_id) is None
        }
    )
...
def test_a_source_row_that_disappears_is_closed_by_the_next_build(tmp_path):
    with GraphStore(tmp_path / "g.db") as s:
        ...
        report = build(s, extractors=[OneEdge(False)], skip_markets=True, prune_on=date(2026, 6, 1))
