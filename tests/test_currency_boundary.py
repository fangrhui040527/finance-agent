...
from engines.sizing.caps import (
    BASE_CURRENCY, Band, BindingCap, CapSet, CurrencyMismatch, concentration_cap,
    cost_floor_value, liquidity_cap, risk_budget_cap, to_base, to_quote,
)
...
def test_a_cap_in_one_currency_cannot_size_a_price_in_another():
    ...
    """
    caps = CapSet(risk=D("40000"), kelly=None, concentration=D("40000"),
                  liquidity=D("999999999"), cost_floor=D("1"))   # MYR by default
    with pytest.raises(CurrencyMismatch, match="priced in USD"):
        size("XNAS:NVDA", Band.ACCUMULATE, D("500000"), caps, D("180"), 1,
             D("165"), BREAKERS, date(2028, 1, 1), XNAS.fee_schedule.round_trip,
             mic="XNAS", currency="USD", fx_base_per_quote=MYR_PER_USD)
...
def test_an_eight_percent_cap_never_funds_a_thirty_percent_position():
    ...
    d = size("XNAS:NVDA", Band.ACCUMULATE, book, caps, D("180"), 1, D("165"),
             BREAKERS, date(2028, 1, 1), XNAS.fee_schedule.round_trip, mic="XNAS",
             currency="USD", fx_base_per_quote=MYR_PER_USD)
...
def test_the_reported_value_is_myr_and_the_traded_value_is_not():
    ...
    caps = CapSet(risk=D("9600"), kelly=None, concentration=D("9523.81"),
                  liquidity=D("999999999"), cost_floor=D("1"), currency="USD")
    d = size("XNAS:NVDA", Band.ACCUMULATE, D("500000"), caps, D("180"), 1,
             D("165"), BREAKERS, date(2028, 1, 1), XNAS.fee_schedule.round_trip,
             mic="XNAS", currency="USD", fx_base_per_quote=MYR_PER_USD)
...
def test_an_myr_market_needs_no_rate_and_reports_one_number():
    ...
    caps = CapSet(risk=risk_budget_cap(inv, D("0.0075"), D("0.125")), kelly=None,
    ...
    d = size("MYX:1155", Band.ACCUMULATE, inv, caps, price, 100, D("5.42"),
...
def test_the_prospective_weight_is_myr_over_myr():
    ...
    existing = [Position("MYX:1155", 0.30, "bank", "MY", "MYR", 0.01),
                Position("MYX:5225", 0.30, "health", "MY", "MYR", 0.01)]
    caps = CapSet(risk=D("30000"), kelly=None, concentration=D("30000"),
                  liquidity=D("999999999"), cost_floor=D("1"), currency="USD")
    # USD 30,000 is MYR 126,000 of a MYR 200,000 book: 63%, far past any limit.
    with pytest.raises(Exception) as e:
        size("XNAS:NVDA", Band.ACCUMULATE, D("200000"), caps, D("180"), 1,
             D("165"), BREAKERS, date(2028, 1, 1), XNAS.fee_schedule.round_trip,
             mic="XNAS", currency="USD", fx_base_per_quote=MYR_PER_USD,
...
def test_a_candidate_defaults_to_its_market_currency_not_to_myr():
    ...
    limit never saw it."""
    caps = CapSet(risk=D("2000"), kelly=None, concentration=D("2000"),
                  liquidity=D("999999999"), cost_floor=D("1"), currency="USD")
    d = size("XNAS:NVDA", Band.ACCUMULATE, D("500000"), caps, D("180"), 1,
             D("165"), BREAKERS, date(2028, 1, 1), XNAS.fee_schedule.round_trip,
             mic="XNAS", currency="USD", fx_base_per_quote=MYR_PER_USD,
             existing=[], limits=Limits(), candidate_meta={"sector": "tech",
                                                           "country": "US"})
    assert d.currency == "USD"
...
def test_a_bursa_only_book_has_no_foreign_currency_exposure():
    ...
    book = [_parse_position("MYX:1155:0.30:bank:MY"),
            _parse_position("MYX:5225:0.30:health:MY")]
...
def test_a_us_holding_still_counts_as_foreign():
    ...
    book = [_parse_position("MYX:1155:0.30:bank:MY"),
            _parse_position("XNAS:NVDA:0.60:tech:US")]
