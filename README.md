...
| A workup always has twelve steps, each with an honest status, and is never a stance | `engines/analysis/workup.py` | `test_workup.py` |
| A peer is a live, citable graph edge, never a guess from a sector label | `knowledge/graph/peers.py` | `test_graph_peers.py` |
| A thesis carries a valuation range the engine derived, or none; the model never types one | `mcp_server/tools.py` | `test_workup.py` |
