...
CURRICULUM: tuple[Concept, ...] = (
    ...
    Concept(
        "compounding",
        Level.L1_MONEY,
        "Compounding and the cost of interruption",
    ...
    Concept(
        "cash_flow",
        Level.L2_STATEMENTS,
        "Cash flow and why it outranks earnings",
    ...
    Concept(
        "restatement",
        Level.L2_STATEMENTS,
        "Restatements and point-in-time truth",
    ...
    Concept(
        "multiples",
        Level.L3_VALUATION,
        "Multiples are shorthand, not analysis",
    ...
    Concept(
        "liquidity",
        Level.L4_PRICE,
        "Liquidity and what your own order does",
    ...
    Concept(
        "volatility",
        Level.L4_PRICE,
        "Volatility as a measurement, not a mood",
    ...
    Concept(
        "trend_vs_noise",
        Level.L4_PRICE,
        "Trend, mean reversion, and noise",
    ...
    Concept(
        ...
        Level.L5_NEWS,
        "Splitting a move before explaining it",
    ...
    Concept(
        "no_catalyst",
        Level.L5_NEWS,
        "The honest answer: no identified catalyst",
    ...
    Concept(
        "position_sizing",
        Level.L6_RISK,
        "Sizing bounds the loss, not the position",
    ...
    Concept(
        "ruin",
        Level.L6_RISK,
        "Risk of ruin and path dependence",
    ...
    Concept(
        "portfolio_heat",
        Level.L7_PORTFOLIO,
        "Total heat across open positions",
    ...
    Concept(
        "currency",
        Level.L7_PORTFOLIO,
        "Currency is a position you did not choose",
    ...
    Concept(
        "journal",
        Level.L8_PROCESS,
        "Writing the thesis before the entry",
    ...
    Concept(
        "breakers",
        Level.L8_PROCESS,
        "Breakers: deciding now how you will be proven wrong",
    ...
    Concept(
        "calibration",
        Level.L8_PROCESS,
        "Calibration: are your 70%s actually 70%?",
...
class A14Teacher(Agent):
    ...
    def run(self, key: str, learner: Learner | None = None) -> list[Finding]:
        ...
        if c.misconception:
            out.append(
                Finding(self.agent_id, "misconception", f"The common error: {c.misconception}")
            )
        if c.check:
            out.append(
                Finding(
            ...
            if lic is Licence.LINK_ONLY:
                out.append(
                    Finding(
                        ...
                        "further_reading",
                        f"{title} - {url}",
                        caveats=["licence permits the link only; the text is not reproduced here"],
                    )
                )
            else:
    ...
    def next_concept(self, learner: Learner) -> Finding:
        ...
        for c in CURRICULUM:
            ...
            if all(r in learner.known for r in c.requires):
                return Finding(
                    self.agent_id, "next", f"next: {c.title} (L{c.level.value}) - {c.one_line}"
                )
        return Finding(
            self.agent_id,
            "next",
