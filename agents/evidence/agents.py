...
class A1Fundamentals(Agent):
    ...
    def earnings_quality(self, instrument_id: str, asof: date) -> list[Finding]:
        ...
        if not (ni and cfo):
            return [Finding(self.agent_id, "quality_flag",
        ...
        return [Finding(
            self.agent_id, "quality_flag",
            ("net income exceeds operating cash flow by more than 30% of earnings"
             if flagged else "net income is broadly supported by operating cash flow"),
            numbers={"accrual_gap": gap},
            caveats=(["most reliable warning sign of aggressive accounting"] if flagged else []),
        )]
...
class A2Valuation(Agent):
    ...
    def run(self, instrument_id: str, archetype: str, current_multiple: float,
            history: list[float]) -> list[Finding]:
        self._guard_tool("multiple_vs_history")
        method, forbidden = self.ARCHETYPE_METHODS.get(
            archetype, ("EV/EBIT vs history", set()))
        band = sorted(history)
        if not band:
            return [Finding(self.agent_id, "valuation",
                            "no multiple history available", caveats=["cannot contextualise"])]
...
class A3PriceTechnical(Agent):
    ...
    def run(self, series: PriceSeries, index_series: PriceSeries | None = None,
            sessions_to_earnings: int | None = None) -> list[Finding]:
        self._guard_tool("ohlcv")
        if len(series) < 21:
            return [Finding(self.agent_id, "price_context",
        ...
        out = [Finding(
            self.agent_id, "price_context",
            f"last close {closes[-1]:.4f}, ATR20 {atr:.4f} ({atr / closes[-1]:.1%} of price), "
            f"{dd:.1%} below the period high, 20-day ADV {series.adv(20):,.0f}",
            numbers={"close": closes[-1], "atr20": atr, "drawdown": dd,
                     "adv20": series.adv(20)},
        )]
        if sessions_to_earnings is not None and abs(sessions_to_earnings) <= self.EARNINGS_BLACKOUT_SESSIONS:
            out.append(Finding(
                self.agent_id, "blackout",
...
class A4NewsNarrative(Agent):
    ...
    def run(self, instrument_id: str, query: str, max_age=None) -> list[Finding]:
        ...
        if res.refused:
            return [Finding(self.agent_id, "news",
        ...
        for hit in res.hits[:5]:
            ...
            out.append(Finding(
                self.agent_id, "news",
                f"{c.text[:160]}",
                citations=[cite("kb_news", c.chunk_id, c.text[:60],
                                TrustTier.CURATED_NEWS, c.as_of or self.ctx.now)],
                as_of=c.as_of,
            ))
        return out
    ...
    def run(self, instrument_id: str, t0: datetime, t1: datetime) -> list[Finding]:
        ...
        for e in window:
            ...
            out.append(Finding(
                ...
                + (f"; historically worth a median {br.median_car:+.2%} (n={br.n})" if br else ""),
...
class A6MacroRegime(Agent):
    ...
    def run(self, market_returns: list[float], vol_window: int = 60) -> list[Finding]:
        ...
        if len(market_returns) < vol_window:
            return [Finding(self.agent_id, "regime", "insufficient history to label a regime",
        ...
        label = "risk_off" if (drift < 0 and vol > 0.22) else "risk_on" if (drift > 0.05 and vol < 0.20) else "neutral"
        return [Finding(
            self.agent_id, "regime",
...
class A8OwnershipFlow(Agent):
    ...
    def run(self, insider_buys: int, insider_sells: int, scheduled_sells: int,
            short_interest_pct: float, days_to_cover: float,
            institutional_asof: date | None = None) -> list[Finding]:
        self._guard_tool("insider_activity")
        out = []
        non_plan = max(0, insider_sells - scheduled_sells)
        if insider_buys >= 3:
            out.append(Finding(self.agent_id, "insider",
        ...
        if non_plan > 0:
            ...
            out.append(Finding(
                self.agent_id, "insider",
                f"{insider_sells} insider sales, all under scheduled plans",
                caveats=["routine insider selling is not treated as a signal: "
                         "plans, tax and diversification dominate the sample"],
            ))
        out.append(Finding(
            self.agent_id, "short_interest",
        ...
        if institutional_asof:
            out.append(Finding(
                self.agent_id, "ownership",
                f"institutional holdings as at {institutional_asof}",
                caveats=[f"institutional filings lag by {self.INSTITUTIONAL_LAG_DAYS}+ days "
                         "by construction"],
            ))
        return out
