...
CSS = (Path(__file__).parent / "_css.txt").read_text(encoding="utf-8")
...
def rail(active):
    ...
    for key, label, icon in NAV:
        if key == "learn":
            out.append('<div class="railsep"></div>')
...
def page(name, active, body, logic=None, props=None, w=1280, h=900):
    ...
    if logic is not None:
        ...
        script = (
            f"<script data-dc-script data-props='{pj}'>\n"
            "class Component extends DCLogic {}\n</script>"
        )
    (Path(__file__).parent / f"{name}.dc.html").write_text(
        f"""<!doctype html>
<html>
<head>
  <meta charset="utf-8">
