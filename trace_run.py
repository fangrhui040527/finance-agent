    ...
    with start_run("full-system") as tracer:
        ...
        with span("inference", kind="span"):
            ...
            client = InferenceClient(
                EchoBackend(), ctx.engine, ledger, daily_budget_myr=cfg.daily_budget_myr
            )
            ...
            try:
                ...
                emit("refusal", "a0_may_not_reason", agent="a0_supervisor", reason=str(e))
            emit(
                "engine",
                "ledger",
                calls=len(list(ledger.calls())),
        ...
        with span("a0_supervisor", kind="agent", agent="a0_supervisor"):
            a0 = A0Supervisor(ctx)
            for q, ids in (
                ("why did maybank fall today", ("MYX:1155",)),
                ...
                if plan.allowed:
                    emit(
                        ...
                        cost_myr=str(plan.estimated_cost.amount),
                        notes=plan.notes,
                    )
                else:
                    emit(
        ...
        with span("a7_sector_technology", kind="agent", agent="a7_sector_technology"):
            ...
            corpus = {
                "doc:port": (
                    "A drone strike closed the Strait of Hormuz to tanker traffic for eleven days."
                ),
                "doc:sector": (
                    "MISC Berhad is classified within the shipping and marine transport sector."
                ),
                "doc:chem": (
                    "Petronas Chemicals is exposed to shipping freight "
            ...
            emit(
                ...
                **{k: v for k, v in report.per_extractor.items()},
                nodes=report.nodes,
                edges=report.edges,
                citable=report.citable,
                hubs=report.hubs or ["none"],
            )
            for nid, kind, lbl in (
                ("EV:hormuz", NodeKind.EVENT, "Strait of Hormuz closure"),
            ...
            for src, dst, kind, doc, conf in (
                ("EV:hormuz", "SEC:shipping", EdgeKind.AFFECTS, "doc:port", Confidence.EXTRACTED),
                (
                    "SEC:shipping",
                    "CO:MISC",
                ...
                ("SEC:shipping", "CO:PCHEM", EdgeKind.EXPOSED_TO, "doc:chem", Confidence.EXTRACTED),
            ...
            emit(
                ...
                **{k: v for k, v in store.counts().items()},
                edges_detail=[
                    f"{e.src} --{e.kind.value}[{e.confidence.value}]--> {e.dst}"
            ...
            emit(
                "engine",
                "graph.answer.extracted",
                agent="a7_sector_technology",
                answered=extracted_answer.answered,
                claims=[
                    {"text": c.text, "quoted": [x.quoted_span for x in c.citations]}
            ...
            emit(
                "refusal",
                "graph.inferred_edge_not_traversed",
                agent="a7_sector_technology",
                reason=(
                    "CO:RUMOUR is reachable but its only edge is INFERRED, so it "
            ...
            emit(
                ...
                claims=[
                    {"text": c.text, "citations": [c2.chunk_id for c2 in c.citations]}
            ...
            refused = blind.emit(
                blind.run("EV:hormuz", holdings, asof=TODAY), chunks, confidence=0.48
            )
            emit(
                "refusal",
                "graph.uncited_path",
        ...
        with span("a9_attribution", kind="agent", agent="a9_attribution"):
            ...
            for label, mv, mkt, sec in (
                ("market-wide fall", -0.090, -0.080, -0.020),
                ...
                exp = decompose("MYX:1155", window, mkt, sec, {}, mv, 0.0, fit, base_currency="MYR")
                emit(
                    "engine",
                    f"attribution.{label}",
                    agent="a9_attribution",
                    scenario=label,
                    move=mv,
                    market=mkt,
                    sector=sec,
        ...
        with span("thesis_and_red_team", kind="span"):
            ...
            breakers = [
                Breaker(statement="NIM falls below 2.0%", query="nim < 0.020", store="kb_filings"),
                Breaker(statement="CASA below 22%", query="casa < 0.22", store="kb_filings"),
            ]
            with span("a10_thesis", kind="agent", agent="a10_thesis"):
                a10 = A10Thesis(ctx)
                out = a10.run(
                    "MYX:1155", evidence, proposed_stance=Stance.ACCUMULATE, breakers=breakers
                )
                thesis = a10.last
                emit(
                    "engine",
                    "thesis",
                    agent="a10_thesis",
                    stance_asked="accumulate",
                    stance_reached=thesis.stance.value,
                    actionable=thesis.is_actionable(),
                    confidence=thesis.confidence,
                    gaps=thesis.gaps,
                    one_sentence=thesis.in_one_sentence,
                    notes=out[0].caveats,
                )
            with span("a11_red_team", kind="agent", agent="a11_red_team"):
                ...
                emit(
                    ...
                    "red_team",
                    agent="a11_red_team",
                    verdict=a11.verdict(challenges),
        ...
        with span("portfolio", kind="span"):
            book = [
                Position("MYX:1155", 0.22, "bank", "MY", "MYR", 0.01),
            ...
            with span("a13_sizing", kind="agent", agent="a13_sizing"):
                ...
                emit(
                    ...
                    "sizing.foreign",
                    agent="a13_sizing",
                    portfolio="MYR 200000",
                    binding_cap=binding.value,
                    cap_value=str(value),
                    currency=caps.currency,
                    cap_value_myr=str(value * Decimal("4.20")),
                    detail=fs[0].text,
                )
                try:
                    a13.caps(
        ...
        with span("a15_reflection", kind="agent", agent="a15_reflection"):
            ...
            emit(
                "engine",
                "prediction.logged",
                agent="a15_reflection",
                prediction_id=p.prediction_id,
                direction=p.direction,
                confidence=p.confidence,
                grade_on=str(p.grade_on),
            )
            try:
            ...
            emit(
                ...
                "lesson.qualifies",
                agent="a15_reflection",
                result=a15.propose("pattern", one, TODAY, {"MYX:1155", "XNAS:NVDA", "XSES:D05"})[
                    0
                ].text,
            )
        ...
        with span("guardrails", kind="span"):
            ...
            for name, action in (
                (
                    "unlisted tool",
                    Action(name="place_order", rail=Rail.TOOL, agent="a1_fundamentals", payload={}),
                ),
                (
                    "injection",
                    Action(
        ...
        if live:
            with span("live_feed", kind="span", instrument=live):
                ...
                feed = default_feed()
                try:
                    series = feed.fetch(live)
                    bars = series.raw()
                    emit(
                        "feed",
                        feed.source_used,
                        source=feed.source_used,
                        instrument=live,
                        count=len(bars),
                        first=str(bars[0].day),
                        last=str(bars[-1].day),
                        adv_20=series.adv(20),
                        atr_20=series.atr(20),
                    )
                except PriceFeedError as e:
                    emit("error", "price_feed", source="chain", error=str(e))
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--live", metavar="INSTRUMENT", help="also fetch real bars, e.g. MYX:1155 (needs network)"
    )
    ...
    for f in ("anatomy.md", "session.log", "report.html", "trace.jsonl", "summary.json"):
        print(f"    {f}")
    print(
        f"    prompts/   ({len(list((__import__('pathlib').Path(d) / 'prompts').iterdir()))} files)"
    )
    if s["errors"]:
        print("\n  ERRORS:")
        for e in s["errors"]:
